...
class baseFunctions: 
    ...
    def makeTablefromCSV(self, fileCSV):
        ...
        use f = open(fileCSV, "r") to read the csv file and make a table. 
        
        Args:
        fileCSV (str): the path to the csv file (of items for sale at the grocery store) to be read in   
        """
        self.df = pandas.read_csv(fileCSV)
        print(self.df.read())
    ...
    def TodaysCheapestItem(self):
        """"Prints the cheapest item for sale at the grocery store today for the user by searching and filtering a dataframe from
        a CSV file (my attribute self.df (which is in the above makeTablefromCSV() function, opens the file for reading). 
        The child function overrides this function and uses min() too, but finds the lowest price and its corresponding item from a
        dictionary by using min(dictionaryName.values() to find the price and iterates through the dictionary using 
        "for key,value in dictionaryName.items(): if todaysMostExpensivePrice == value:" to find the name of the cheapest item.
        """
        lowestPrice = self.df['Price'].min()
        filtered_df_lowest = self.df.query(lowestPrice)
        filtered_df_lowest['itemName']
        leastExpensiveItemToday= filtered_df_lowest['itemName'].iloc[0]
        cheapestItemTodayWithPrice = f"The most expensive item today is {leastExpensiveItemToday}. which costs ${lowestPrice}"
        print(cheapestItemTodayWithPrice)
    
    def TodaysMostExpensiveItem(self):
        """Prints the most expensive item for sale at the grocery store today for the user by searching and filtering a dataframe from
        a CSV file (my attribute self.df (which is in the above makeTablefromCSV() function, opens the file for reading). 
        The child function overrides this function and uses max() too, but finds the hghest price and its corresponding item from a
        dictionary by using max(dictionaryName.values() to find the price and iterates through the dictionary using 
        "for key,value in dictionaryName.items(): if todaysMostExpensivePrice == value:" to find the name of the most expensive item.
        """
        highestPrice = self.df['Price'].max()
        filtered_df_highest = self.df.query(highestPrice)
        filtered_df_highest['itemName']
        mostExpensiveItemToday= filtered_df_highest['itemName'].iloc[0]
        mostPriceyItemTodayWithPrice = f"The most expensive item today is {mostExpensiveItemToday}. which costs ${highestPrice}"
        print(mostPriceyItemTodayWithPrice)
